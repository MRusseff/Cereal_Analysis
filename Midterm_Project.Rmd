...
mtp_hyp1 %>%
  ...
  summarise(m = mean(revenue), sd = sd(revenue), 
            n = n(), ci = z * sd/sqrt(n)) %>%
  ggplot(aes(x = reorder(cereal, m), y = m, fill = reorder(company, m))) +
...
#Combining the three graphs into one like above but with each company ordered
grid.arrange(
  ...
  mtp_hyp1 %>%
    ...
    geom_bar(stat = "identity", position = "dodge", fill = "#deebf7") +
    coord_flip() +
    theme_classic() +
    xlab("") +
    labs(fill = "") +
...
mtp_hyp1 %>%
  group_by(company, cereal) %>%
  summarise(m = mean(price), sd = sd(price), 
            n = n(), ci = z * sd/sqrt(n)) %>%
  ggplot(aes(x = cereal, y = m, fill = company)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = m - ci, ymax = m + ci), 
                width = 0.5, position = position_dodge(0.9)) +
  coord_flip() +
